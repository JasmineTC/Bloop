...
def replaceSymbolsWithIndices(expression, symbols):
    ...
    for idx, symbol in enumerate(
        sorted(symbols, reverse=True, key=lambda symbol: replaceGreekSymbols(symbol))
    ):
...
def parseExpressionArray(line, allSymbols):
    ...
    identifier, expression = (
        map(str.strip, line.split("->")) if "->" in line else ("anonymous", line)
    )

    expression = parse_mathematica(expression)

    return {
        "identifier": removeSuffices(identifier),
...
def parseMassMatrix(definitionsLines, matrixLines):
    ...
    return {
        "definitions": parseExpressionSystem(definitionsLines),
